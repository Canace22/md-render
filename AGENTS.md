...
| `md-render-excalidraw-canvas` | Excalidraw 无限画布的同步、保存与交互边界 |
...
| `md-render-electron-startup` | Electron 开发启动、白屏、preload 与 ESM/CJS 崩溃诊断 |
...
- 产品长期目标是沉淀可复用资产，而不是把数据锁死在某个页面或流程里。
- 资产包括但不限于：文档、图片/音视频、摘录、知识库条目、模板、发布配置、Daily 记录、AI 生成结果、外部来源导入内容。
- 新功能先判断属于哪类边界：新资产类型、资产加工动作、业务视图/工作流、外部来源适配器、发布/同步适配器。
- 资产应尽量有稳定 `id`、类型、元数据、来源/派生关系、引用关系和更新时间；不要只存在于组件局部状态或某个页面的私有结构里。
- 业务流程只做编排：读取资产、生成资产、关联资产、更新状态；不要跨模块直接依赖其他页面的内部数据结构。
- 平台化生成、改写、摘要、发布稿等默认生成新资产或派生版本，保留原稿；只有用户明确要求覆盖时才修改源资产。
- 外部来源和平台差异在 adapter/service 层收口，进入应用内部后尽量转换为统一资产语义。
- 不提前设计庞大的通用资产框架；先沿现有 store、utils/service、IPC 边界做最小抽象，重复和稳定契约出现后再提炼。
