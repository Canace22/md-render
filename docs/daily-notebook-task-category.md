...
「今日速记」把每天的内容分成三种类型，它们**共用「今日记录」这一个列表**，靠行首的类型标签区分：
...
任务、笔记、事件合并在「今日记录」这一个列表里，点右上角 **「+」** 选择要新建的类型：

1. 在列表顶部出现一行待保存的草稿，行首是 **类型标签**
2. 草稿行内可选 **优先级**（仅任务）与 **类别**（任务 / 笔记）
3. 输入文字后回车或点「保存」；留空点「删除」即丢弃草稿

### 3.2 新建笔记

与任务同一个入口：「+」→ 选「笔记」，草稿行内可选类别。事件不带类别，只填文字。
...
合并列表的排序：**未完成在前、已完成沉底**；未完成里按 任务 → 事件 → 笔记 分段，任务段内按优先级（高 → 中 → 低），同优先级按创建时间。列表顶部可按**类型**筛选，**仍不按类别分组或筛选**。
...
useEditorStore.js          addDailyItem(date, type, text, category?)
       ...
       ↓
DailyNotebook.jsx          容器：日期、行内草稿、类型筛选、把 store 动作分发给两列
  daily/DailyEntryList.jsx   今日记录单列表：类型筛选、批量改日期、空态
  daily/DailyItemRow.jsx     单条：类型 Tag、优先级点、类别 Tag / 下拉、行内编辑
  daily/DailyTodoColumn.jsx  待办池：类别 Tag / 下拉、加入今天、移除
  daily/dailyOptions.jsx     类型 / 优先级选项、排序比较器、下拉菜单构造
MarkdownEditor.jsx         把 store 动作传给 DailyNotebook
styles.css                 .daily-notebook-category-* / .daily-notebook-type-* 样式
```
...
| 项 | 现状 | 可扩展方向 |
|----|------|------------|
| 筛选 / 分组 | 今日记录支持按**类型**筛选，无类别筛选 | 类型筛选旁再加一排类别 chip；待办池同理 |
