...
4. 来源关系写进 `sourceMaterialIds`；如果来自当前文档，让 store action 自动补当前文档 id。
5. 平台、状态、摘要等元数据放进 `meta`，由 `createDefaultKnowledgeFields` 归一化。
6. UI 只展示和触发动作；资产创建、派生、关联逻辑放在 store action 或 utils/service。
7. 如果是 AI 助手链路，`toolRegistry.js` 只定义工具和转发参数，`AgentPanel.jsx` 的 host 再对接 store。
