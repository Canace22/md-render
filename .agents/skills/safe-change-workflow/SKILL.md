...
description: 在本项目（md-render）改动代码前后的标准安全流程——定向搜索、看懂相关代码、最小化改动、列 10 条 case 预期。默认不要主动跑测试，尤其不要主动跑 E2E；只有用户明确要求时才执行测试命令。任何涉及修改 src/ 下源码、修 bug、加功能、重构的任务都应该走这个流程，即使用户没有明说"要安全"。
---
...
默认只列建议验证项，不主动执行测试命令。用户明确要求“跑测试”“帮我验证”“跑单测”时，优先跑最小范围的 `pnpm test:unit` 或用户指定命令。
